...
public class AuthController {
        ...
        public ResponseEntity<AppResponse<Object>> register(@Valid @RequestBody RegisterRequest request) {
                authService.register(request);
                AppResponse<Object> response = new AppResponse<>("SUCCESS",
        ...
        @ApiResponses(value = {
                        ...
                        @ApiResponse(responseCode = "404", description = "Token not found or invalid", content = @Content)
        })
        @GetMapping("/verify-email")
        public ResponseEntity<AppResponse<String>> verifyEmail(@RequestParam("token") String token) {
                String resultMessage = authService.verifyEmail(token);
                AppResponse<String> response = new AppResponse<>("SUCCESS", resultMessage, null, null);
        ...
        @ApiResponses(value = {
                        ...
                        @ApiResponse(responseCode = "401", description = "Invalid credentials or account not active", content = @Content)
        })
        @PostMapping("/login")
        public ResponseEntity<AppResponse<AuthResponse>> login(@Valid @RequestBody LoginRequest request) {
                AuthResponse authResponse = authService.login(request);
                AppResponse<AuthResponse> response = new AppResponse<>("SUCCESS", "User logged in successfully.",
                                authResponse,
                                null);
                return ResponseEntity.ok(response);
        }
        ...
        public ResponseEntity<AppResponse<AuthResponse>> refreshToken(@Valid @RequestBody RefreshTokenRequest request) {
                AuthResponse authResponse = authService.refreshToken(request);
                AppResponse<AuthResponse> response = new AppResponse<>("SUCCESS", "Token refreshed successfully.",
                                authResponse,
                                null);
                return ResponseEntity.ok(response);
        }
